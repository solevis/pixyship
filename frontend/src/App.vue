<template>
  <v-app>
    ...
    <v-app-bar app ref="toolbar">
      <v-app-bar-nav-icon v-if="display.smAndDown.value" aria-label="Menu" @click="drawer = true"></v-app-bar-nav-icon>

      <v-toolbar-title v-if="display.smAndDown.value || display.lgAndUp.value">
        <router-link to="/">
          <v-img
            src="@/assets/img/pixyship.svg"
            max-height="57"
            max-width="150"
            contain
            alt="PixyShip logo"
      ...
      <v-toolbar-title v-else>
        <router-link to="/">
          <v-img
            src="@/assets/img/pixyship_logo.svg"
            max-height="40"
            max-width="40"
            class="mr-2"
            contain
...
export default {
  setup() {
    useHead({
      title: 'PixyShip',
      titleTemplate: (title) => title ? `PixyShip - ${title}` : 'PixyShip',
      meta: [
        { charset: 'utf-8' },
        {
          name: 'description',
